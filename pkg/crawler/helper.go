...
func (c Crawler) makeDNS() ([]string, error) {
	bootnodesDNS := []string{params.KnownDNSNetwork(core.GenesisToBlock(c.makeGenesis(), nil).Hash(), "all")}
	if c.Classic {
		bootnodesDNS = []string{params.ClassicDNSNetwork1}
	}
	if c.Mordor {
		bootnodesDNS = []string{params.MordorDNSNetwork1}
	}
	if len(c.DNSBootnodes) != 0 {
		bootnodesDNS = c.DNSBootnodes
	}
	return bootnodesDNS, nil
}
